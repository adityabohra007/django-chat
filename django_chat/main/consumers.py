...
#            if self.room_id and
    ...
    async def live_method(self):
        serializer = ChatMemberSerializer(User.objects.filter(
            room__id=self.room__id).exclude(client__id=None), many=True).data
        return json.loads(json.dumps(serializer))

    async def all_method(self):
        serializer = ChatMemberSerializer(
            User.objects.filter(room__id=self.room__id), many=True).data
        return json.loads(json.dumps(serializer))
...
#                pass
    ...
    async def fetch_messages(self, room_id, last_fetch, fetch_count):
        data = None
        if last_fetch:
            data = Room.objects.get(id=self.room.id).message_set.filter(
                id__lt=last_fetch).order_by('date_modified')
        else:
            data = Room.objects.get(
                id=self.room.id).message_set.filter().order_by('date_modified')
        if data.count()-fetch_count >= 1:
            self.moreAvailable = True
        else:
            self.moreAvailable = False
        messages = data.values('id', 'text', 'date_created',
                               'sender', 'sender_id', 'sender__username', 'file')[:]
        newdic = []
        temp_last_fetch = self.last_fetch
        for message in messages:
            if message['id'] < temp_last_fetch or temp_last_fetch == 0:
                temp_last_fetch = message['id']
            message['date_created'] = message['date_created'].strftime(
                '%m/%d/%Y %H:%M:%S %p')
    ...
    async def receive_json(self, data):
        ...
        if message_type == "file":
            ...
            try:
                # {"type":"send.websocket'})
                await self.channel_layer.group_send(self.room_group_name, new_message)
            except:
        ...
        if message_type == "text":
            print("got something ----- ", data,
                  (data['sender'] != self.user.username), (data['room_id'] != str(self.room.id)))
            try:
